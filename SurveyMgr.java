...
public class SurveyMgr {
	...
	public Vector<SurveyBean> listSurveys() {
	    ...
	    String sql = "SELECT s.poll_id, s.question, s.sdate, s.edate, s.created_at, " +
	                 "CASE WHEN s.edate < CURDATE() THEN 0 ELSE s.type END AS type, " +
	                 "s.active, i.itemlist_num, i.item, i.vote_count " +
	                 "FROM tblpolllist s " +
	    ...
	    try {
	        ...
	        while (rs.next()) {
	            ...
	            currentSurvey.setSurveyType(rs.getInt("type"));
	            currentSurvey.setIsActive(rs.getInt("active"));
	...
	// 설문 생성 메소드
	public boolean createSurvey(String question, Date endDate, boolean isMultiSelect, Vector<String> items) {
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		String sql = "INSERT INTO tblpolllist (question, sdate, edate, type, active) VALUES (?, ?, ?, ?, 1)";
		boolean flag = false;
		...
		try {
			...
			pstmt.setInt(4, isMultiSelect ? 2 : 1); // 1: 단일 선택, 2: 복수 선택
	...
	public SurveyBean getSurveyDetail(int pollId) {
	    ...
	    String sql = "SELECT s.poll_id, s.question, s.sdate, s.edate, s.type, s.active, "
	    ...
	    try {
	        ...
	        while (rs.next()) {
	            if (survey == null) {
	                ...
	                survey.setSurveyType(rs.getInt("type"));
	                survey.setIsActive(rs.getInt("active"));
	            }

...
public class SurveyPageAWT extends JFrame implements ActionListener {
	...
	private void createSurveyList() {
		...
		int ongoingCount = (int) surveys.stream().filter(s -> s.getSurveyType() != 0).map(SurveyBean::getPollId)
				.distinct().count();
		...
		for (SurveyBean survey : surveys) {
			if (survey.getSurveyType() != 0 && !addedSurveyIds.contains(survey.getPollId())) { // 진행 중인 설문만 추가
		...
		int finishedCount = (int) surveys.stream().filter(s -> s.getSurveyType() == 0).map(SurveyBean::getPollId)
				.distinct().count();
		...
		for (SurveyBean survey : surveys) {
			if (survey.getSurveyType() == 0 && !addedSurveyIds.contains(survey.getPollId())) { // 종료된 설문만 추가
